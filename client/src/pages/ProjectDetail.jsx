...
export default function ProjectDetail() {
  ...
  const handleAnalyzeAll = async () => {
    if (
      !confirm(
        `This will analyze all unanalyzed feedback using AI. This may take a few minutes and will use Claude API credits. Continue?`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      alert(
        `Analysis complete! Successfully analyzed ${result.analyzed} out of ${result.total} feedback items.`
      );
  ...
  return (
    <DashboardLayout>
      <div className="px-4 sm:px-0">
        ...
        {activeTab === "upload" && (
          <UploadCSV projectId={id} onUploadComplete={handleUploadComplete} />
        )}

        {activeTab === "manual" && (
          <ManualInput projectId={id} onFeedbackAdded={handleFeedbackAdded} />
        )}
      </div>
    </DashboardLayout>
  );
